...
def query1(C,O):
    ...
    row=cursor.fetchall();
    random_input=row[0]['customer_postal_code']
    # run the final query using the input tables and the random customer postal code
    cursor.execute("""SELECT Count(order_id)
    FROM "{}" C, "{}" O
    where C.customer_postal_code=? 
    AND C.customer_id=O.customer_id;""".format(C.replace('"', '""'),O.replace('"', '""')),(random_input,))
...
def runSmall():
    ...
    start=time.time()
    for i in range(0,50):
        query1("Customers_uninformed","Orders_uninformed")
