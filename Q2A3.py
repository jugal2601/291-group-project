...
def query2(C,O, OI):
    ...
    # create the view
    cursor.execute("""CREATE VIEW IF NOT EXISTS OrderSize AS
...
def runSmall():
    ...
    start=time.time()
    for i in range(0,50):
        query2("Customers_uninformed","Orders_uninformed","Order_items_uninformed")
    ...
    start_02=time.time()
    for i in range(0,50):
        query2("Customers","Orders","Order_items")
...
def runLarge():
    ...
    start_03=time.time()
    for i in range(0,50):
        query2("Customers","Orders","Order_items_uninformed")
    # end_03 stores the end time
    end_03=time.time()
    connection.close()
    # calculate the average runtime and store it in the list
